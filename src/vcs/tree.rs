...
pub fn list_files(root_path: &Path, max_files: usize) -> Vec<String> {
    // 1. On configure le WalkBuilder EXACTEMENT comme dans ton module commit
    let walker = WalkBuilder::new(root_path)
        .hidden(false)
        .add_custom_ignore_filename(".awqignore") // <-- C'est ici la correction principale !
        .parents(true)
        .git_ignore(false) // AWQ est indépendant de Git
        .git_exclude(false)
        .threads(4)
        .build();
    ...
    // 2. On utilise flatten() pour zapper proprement les erreurs et avoir un DirEntry direct
    for result in walker.flatten() {
        ...
        // 3. On ignore manuellement les dossiers VCS profonds et on s'assure de ne lister que les fichiers
        ...
        // 4. On récupère le chemin relatif sous forme de String
